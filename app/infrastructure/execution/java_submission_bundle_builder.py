...
#
#        for index, test_case in enumerate(test_cases, start=1):
#            escaped_input = self._escape_shell_string(
#                test_case.input_data
#            )
#
#            escaped_expected = self._escape_shell_string(
...
#                    f"'{escaped_input}' | java Main | tr -d '\\r')"
#                ),
#                (
#                    "expected_output=$(printf '%s' "
#                    f"'{escaped_expected}' | tr -d '\\r')"
#                ),
#                (
#                    "normalized_actual=$(printf '%s' "
#                    '"$actual_output" | sed \'s/[[:space:]]*$//\')'
#                ),
#                (
#                    "normalized_expected=$(printf '%s' "
#                    '"$expected_output" | sed \'s/[[:space:]]*$//\')'
#                ),
#                'if [ "$normalized_actual" != "$normalized_expected" ]; then',
#                (
#                    f'  echo "Test case {index} failed: '
#                    'expected \'$normalized_expected\', '
#                    'got \'$normalized_actual\'" >&2'
#                ),
#                '  exit 1',
#                'fi',
#                '',
