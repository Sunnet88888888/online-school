...
async def create_course(request: CreateCourseRequest, use_case: CreateCourseUseCase = Depends(get_create_course_use_case),) -> CourseResponse:
...
async def updaet_course(course_id:UUID, request: UpdateCourseRequest, use_case: UpdateCourseUseCase = Depends(get_update_course_use_case),) -> CourseResponse:
...
async def create_module(course_id: UUID, request: CreateModuleRequest, use_case: CreateModuleUseCase = Depends(get_create_module_use_case),) -> ModuleResponse:
    result = await use_case.execute(
        CreateModuleCommand(
            course_id = course_id,
            title=request.title,
...
async def update_module(module_id: UUID, request: UpdateModuleRequest, use_case: UpdateModuleUseCase = Depends(get_update_module_use_case),) -> ModuleResponse:
    result = await use_case.execute(UpdateModuleCommand(
        module_id = module_id,
        title=request.title,
        description=request.description,
        position = request.position,
...
async def update_section(
    ...
    use_case: UpdateSectionUseCase = Depends(get_update_section_use_case),) -> SectionResponse:
    
    result = await use_case.execute(UpdateSectionCommand(section_id=section_id, title=request.title, description=request.description, position=request.position,))
...
async def create_lecture(section_id: UUID, request: CreateLectureRequest, use_case: CreateLectureUseCase = Depends(get_create_lecture_use_case),) -> LectureResponse:
...
async def update_lecture(lecture_id: UUID, request: UpdateLectureRequest, use_case: UpdateLectureUseCase = Depends(get_update_lecture_use_case),) -> LectureResponse:
